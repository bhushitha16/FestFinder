...
import { eq, count, inArray, gte } from "drizzle-orm";
import { getSessionUser } from "../lib/auth.js";
import { sendCollegeApprovalEmail } from "../lib/mail.js";
...
router.get("/admins/pending", async (_req, res, next) => {
  try {
...
router.put("/admins/:adminId/approve", async (req, res, next) => {
  try {
    ...
    let collegeName = adminProfile[0].collegeName;

    let college = await db.select().from(collegesTable).where(eq(collegesTable.name, collegeName)).limit(1);
    if (!college.length) {
      const [created] = await db.insert(collegesTable).values({ name: collegeName, status: "active" }).returning();
      if (!created) throw new Error("Failed to create college");
    ...
    await db.update(adminProfilesTable).set({ collegeId: collegeId }).where(eq(adminProfilesTable.userId, adminId));

    await db.update(usersTable).set({ status: "active", updatedAt: new Date() }).where(eq(usersTable.id, adminId));

    const updatedRows = await db.select().from(usersTable).where(eq(usersTable.id, adminId)).limit(1);
    const profileRows = await db.select().from(adminProfilesTable).where(eq(adminProfilesTable.userId, adminId)).limit(1);

    if (updatedRows.length && updatedRows[0].email) {
      // Background email
      sendCollegeApprovalEmail(updatedRows[0].email, profileRows[0]?.collegeName ?? "Your College")
        .catch(e => console.error("Error sending college approval email:", e));
    }

    if (!updatedRows.length) throw new Error("Failed to retrieve updated admin user");

    return res.json({
      id: updatedRows[0].id,
      name: updatedRows[0].fullName,
      email: updatedRows[0].email,
      contactNumber: updatedRows[0].contactNumber ?? "",
      collegeName: profileRows[0]?.collegeName ?? "",
...
router.put("/admins/:adminId/reject", async (req, res, next) => {
  try {
    const adminId = parseInt(req.params.adminId);
    if (isNaN(adminId)) return res.status(400).json({ error: "Invalid admin ID" });
    ...
    if (!updated.length) return res.status(404).json({ error: "Admin not found after update" });
...
router.get("/stats", async (_req, res, next) => {
  try {
    ...
    return res.json({
      ...
      totalColleges: Number(collegesCount?.count) ?? 0,
      totalEvents: Number(eventsCount?.count) ?? 0,
      totalRegistrations: Number(registrationsCount?.count) ?? 0,
      pendingAdminRequests: Number(pendingCount?.count) ?? 0,
    });
  } catch (err) {
    next(err);
  }
...
router.get("/events", async (_req, res, next) => {
  try {
    const events = await db
      ...
      .where(gte(eventsTable.eventDate, new Date(new Date().setMonth(new Date().getMonth() - 6))));
