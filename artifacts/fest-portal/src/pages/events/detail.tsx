...
export default function EventDetail({ id }: { id?: string }) {
  ...
  const addBookmark = useAddBookmark({ mutation: { onSuccess: () => queryClient.invalidateQueries({ queryKey: ["/api/bookmarks"] }) }});
  const removeBookmark = useRemoveBookmark({ mutation: { onSuccess: () => queryClient.invalidateQueries({ queryKey: ["/api/bookmarks"] }) }});

  return (
    <AppLayout>
      <div className="max-w-5xl mx-auto px-4 py-8 w-full">
        <Link href={isStudent ? "/student/dashboard" : "/"} className="inline-flex items-center text-sm text-muted-foreground hover:text-white mb-6 transition-colors">
        ...
        <div className="relative h-[40vh] md:h-[50vh] w-full rounded-2xl overflow-hidden mb-10 border border-white/10 shadow-2xl">
          ...
          <div className="absolute bottom-0 left-0 p-8 w-full">
            <div className="flex flex-wrap items-center gap-3 mb-4">
              ...
              <Badge variant={isCompleted ? "default" : "warning"} className="px-4 py-1.5 text-sm uppercase tracking-wider">{event.eventStatus}</Badge>
            </div>
          ...
          {isStudent && (
            <div className="absolute top-6 right-6">
              <button 
                onClick={() => isBookmarked ? removeBookmark.mutate({ eventId: event.id }) : addBookmark.mutate({ eventId: event.id })}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          <div className="md:col-span-2 space-y-10">
            ...
            {isCompleted && event.photos && event.photos.length > 0 && (
              <section>
                <h2 className="text-2xl font-display font-semibold mb-6 text-white border-b border-white/5 pb-4">Event Gallery</h2>
                <div className="grid grid-cols-2 gap-4">
                  {event.photos.map(photo => (
                    <div key={photo.id} className="relative aspect-video rounded-xl overflow-hidden group bg-secondary">
                      <img src={photo.photoUrl} alt="Event" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                      {photo.caption && (
                ...
                <div className="flex items-end justify-between mb-6 border-b border-white/5 pb-4">
                  ...
                  {event.averageRating && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-muted-foreground font-medium">({event.averageRating.toFixed(1)}) • {event.reviewCount} reviews</span>
                    </div>
                  )}
                </div>
                  ...
                  <Card className="mb-8 p-6 bg-secondary/30">
                    ...
                    <div className="space-y-4">
                      ...
                      <Button 
                        ...
                        onClick={() => submitReview.mutate({ eventId: event.id, data: { rating: reviewRating, review: reviewText } })}
                ...
                <div className="space-y-4">
                  {event.reviews?.length > 0 ? event.reviews.map(review => (
                    <div key={review.id} className="p-6 rounded-xl bg-card border border-white/5">
                      ...
                      {review.review && <p className="text-muted-foreground text-sm italic">"{review.review}"</p>}
                      <p className="text-xs text-muted-foreground/50 mt-4">{format(new Date(review.createdAt), 'MMMM dd, yyyy')}</p>
                    </div>
                  )) : (
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground mt-0.5">{format(new Date(event.eventDate), 'EEEE, MMMM dd, yyyy')}</p>
                ...
                <div className="flex items-start gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground mt-0.5">{event.registeredCount} {event.maxParticipants ? `/ ${event.maxParticipants}` : ''} Attending</p>
                    <p className="text-xs text-destructive mt-1">Closes: {format(new Date(event.registrationDeadline), 'MMM dd, h:mm a')}</p>
                  </div>
                </div>
              </div>
              ...
              {isStudent ? (
                ...
                <div className="pt-6 border-t border-white/10 text-center">
                  <Link href="/student/login">
                    <Button className="w-full" variant="outline">Login to Register</Button>
                  </Link>
                </div>
              ) : null}
            </Card>
